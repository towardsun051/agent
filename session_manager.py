...
class SessionManager:
    ...
    def compress_history(
        ...
        # Append summary to compressed_context.
        # 注意（教学版局限）：多次压缩时摘要会累积拼接（existing + "\n---\n" + summary），
        # compressed_context 会越压越长，与"压缩应缩短上下文"的初衷相悖。
        # 生产级实现（如 openclaw）用指针只保留最后一次有效摘要；此处为教学简化保留累积写法。
